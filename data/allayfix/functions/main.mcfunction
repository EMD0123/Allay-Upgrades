...
execute if score amethyst_pad allayfix.gamerules matches 1 as @e[type=minecraft:allay] at @s unless data entity @s HandItems[0].id if predicate allayfix:amethyst_pad run attribute @s minecraft:generic.flying_speed modifier add eae3f3f1-bc38-4a3c-b45d-5cca9eb97fe3 allayfix.amethyst_pad -1 add_multiplied_total
...
#book
execute if score allaybook allayfix.gamerules matches 1 as @e[type=item,nbt={Item:{id:"minecraft:book",count:1}},tag=!allayfix.used] at @s at @e[type=minecraft:allay,distance=0..1,limit=1,sort=nearest] run function allayfix:allaybook/book
#unbook
